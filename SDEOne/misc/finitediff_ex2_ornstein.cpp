// to run to sourceCpp("finitediff_ex2.cpp")
// then call m<-ode(inits,-2.0,3.0,1.0,1.0,100,(4- -4)/100)
...
#ifdef IGNORE
inits<-rep(0,101);
inits[43:59]<-c( 0.000100744,  0.00111052,
 0.00888912, 0.0516675, 
 0.218073, 0.668361, 
 1.48746,  2.40385, 
 2.82095,  2.40385, 
 1.48746,  0.668361, 
 0.218073,  0.0516675, 
 0.00888912, 0.00111052, 
 0.000100744)
#endif  
...
class f_ode {
    ...
    //members
    double m_mu,m_theta,m_sigma2;                     //misc model parameters
...
public:
    f_ode( double mu,
           ...
           vector<double> &vec_xgrid): m_mu(mu), 
                                       m_theta(theta), 
                                             m_sigma2(sigma2),
                                             m_h(h),
                                             m_n(n),
    ...
    /******************* START OF KEY SECTION = THE MODELLING PART WHICH SHOULD BE CUSTOMIZED **************************/
      ...
      coef1_0 = (-m_mu+m_vec_xgrid[0]-(25.*m_sigma2/24.*m_h));
      coef2_0 = 2.*m_sigma2*dxdt[1]/m_h - 3.*m_sigma2*dxdt[2]/2.*m_h + 2.*m_sigma2*dxdt[3]/3.*m_h - m_sigma2*dxdt[4]/8.*m_h;
      coef3_0 = 0.5*m_sigma2*(-25.*x[0]/12.*m_h + 4.*x[1]/m_h - 3.*x[2]/m_h + 4.*x[3]/3.*m_h - x[4]/4.*m_h);
      coef4_0 = (-m_mu+m_vec_xgrid[0])*x[0];                        
        
        
      //Rcout<<1./coef1_0<<" "<<coef2_0<<" "<<coef3_0<<" "<<coef4_0<<endl;  
      dxdt[0] = (-m_sigma2*(-48.*dxdt[1]+36.*dxdt[2]-16.*dxdt[3]+3.*dxdt[4]))/(24.*m_h*(m_mu-m_vec_xgrid[0])+25.*m_sigma2);
      //dxdt[0]=0.0;
        ...
        for(int r=1;r<(m_n-1);r++){//e.g. m_n=11 for n=10 steps
          ...
          for(int c=0;c<m_vec2d_xx[r].size();c++){ //add up all the relevant terms using the cofficients stored in m_vec2d[] and m_vec2dh
            ...
            // this is PDE specific - tailor to the equation, this line is for f_xx 
              dxdt[r] += (m_sigma2/2.)*(m_vec2d_xx[r][c]*x[m_vec2dh_xx[r][c]]/(m_h*m_h)); // f_t = (sigma2/2)*f_xx ...
            }
          
          for(int c=0;c<m_vec2d_x[r].size();c++){ //add up all the relevant terms using the cofficients stored in m_vec2d[] and m_vec2dh
          
           // this is PDE specific - tailor to the equation, this line is for u_x terms  
              dxdt[r] += -m_theta*(m_mu-m_vec_xgrid[m_vec2dh_x[r][c]])*m_vec2d_x[r][c]*x[m_vec2dh_x[r][c]]/m_h; // f_t = ... - theta*f2_x where f2= (mu-x_i)f so D[(mu-x_i)f,x]
            }
      ...
      coef1_1 = (-m_mu+m_vec_xgrid[m_n-1]+(25.*m_sigma2/24.*m_h));
      coef2_1 =  m_sigma2*dxdt[m_n-5]/8.*m_h - 2.*m_sigma2*dxdt[m_n-4]/3.*m_h + 3.*m_sigma2*dxdt[m_n-3]/2.*m_h -2.*m_sigma2*dxdt[m_n-2]/m_h   ;
      coef3_1 = 0.5*m_sigma2*(x[m_n-5]/4.*m_h - 4.*x[m_n-4]/3.*m_h + 3.*x[m_n-3]/m_h - 4.*x[m_n-2]/m_h + 25.*x[m_n-1]/12.*m_h);
      coef4_1 = (-m_mu+m_vec_xgrid[m_n-1])*x[m_n-1];                        
        
      dxdt[m_n-1] = -m_sigma2*(-3.*dxdt[m_n-5]+16.*dxdt[m_n-4]-36.*dxdt[m_n-3]+48.*dxdt[m_n-2])/(24.*m_h*(m_mu-m_vec_xgrid[m_n-1])-25.*m_sigma2);  
...
// [[Rcpp::export]]
NumericMatrix ode(NumericVector x0,double mu, double theta, double sigma2, double tend,int nn, double h) {
  ...
  double lowerX=-4;//lower end of x
  int i=0;
 ...
 //Rcpp::stop("stopping\n"); 
  ...
  //          x , 0.0 , 4.0 , 0.1 );
    ...
    vector<double> times;
f_ode myf(mu,theta,sigma2,h,n,myvec_xx,myvech_xx,myvec_x,myvech_x,myvec_xgrid);
