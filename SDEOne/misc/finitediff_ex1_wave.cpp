// to run to sourceCpp("finitediff_ex1.cpp")
// then call m<-ode(rep(0,11),0.25,2,4.0,10,(1/10))
...
class f_ode {
    ...
    //members
    double m_k, m_r;                     //misc model parameters
...
public:
    f_ode( double k, 
           ...
           vector<vector<int> > &vec2dh_x) : m_k(k), 
                                             m_r(rr),
                                             m_h(h),
                                             m_n(n),
                                             m_vec2d_xx(vec2d_xx),
    ...
    /******************* START OF KEY SECTION = THE MODELLING PART WHICH SHOULD BE CUSTOMIZED **************************/
        ...
        dxdt[0] = 2*M_PI*cos(2*M_PI*t)+sin(2*M_PI*t)-x[0];
        ...
        for(int r=1;r<(m_n-1);r++){//e.g. m_n=11 for n=10 steps
          ...
          for(int c=0;c<m_vec2d_xx[r].size();c++){ //add up all the relevant terms using the cofficients stored in m_vec2d[] and m_vec2dh
            ...
            // this is PDE specific - tailor to the equation, this line is for u_xx - different FD scheme for u_x
              dxdt[r] += (1./8)*(m_vec2d_xx[r][c]*x[m_vec2dh_xx[r][c]]/(m_h*m_h)); // u_t = (1/8) u_xx ...
            }
          ...
          for(int c=0;c<m_vec2d_x[r].size();c++){ //add up all the relevant terms using the cofficients stored in m_vec2d[] and m_vec2dh
           ...
           // this is PDE specific - tailor to the equation, this line is for u_x terms  
              dxdt[r] += -m_vec2d_x[r][c]*x[m_vec2dh_x[r][c]]/m_h; // u_t = ... - u_x
            }
          
        } // end of row loop, i.e. ODEs in the body
        
       // CUSTOM BOUNDARY - UPPER in space dimension
       dxdt[m_n-1] = -((12*m_h)/(25))*( dxdt[m_n-5]/(4*m_h) - 4*dxdt[m_n-4]/(3*m_h)  + 3*dxdt[m_n-3]/m_h - 4*dxdt[m_n-2]/m_h);
...
// [[Rcpp::export]]
NumericMatrix ode(NumericVector x0,double k, double rr, double tend,int nn, double h) {
 ...
 for (auto it = myvech_x.begin();it != myvech_x.end(); it++){
  ...
  //          x , 0.0 , 4.0 , 0.1 );
    ...
    vector<double> times;
f_ode myf(k,rr,h,n,myvec_xx,myvech_xx,myvec_x,myvech_x);
