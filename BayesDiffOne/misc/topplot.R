setwd("/home/lewisfa/dstools.DSblog1");

load("simsData.RData"); # provides mydat.df

data1<-split(mydat.df,list(mydat.df$set))[[1]]
names(data1)[2:3]<-c("Time","Observation");
data1<-data1[,-1];#drop set

data2<-data.frame(Time=seq(23.1,25.0,by=0.1),Observation=NA)
data1<-rbind(data1,data2);

# make into lagged - t+1|t
data1b<-cbind(data1[1:(nrow(data1)-1),],data1[2:nrow(data1),])
names(data1b)[c(1:4)]<-c("Time1","Observation1","Time2","Observation2")
data1b[232:250,"Observation1"]<-data1b[231,"Observation1"]
data1b[232:250,"Time1"]<-data1b[231,"Time1"]

data1<-data1b;

fits<-data1[c(20,34,70,90,122,150,174,190,210,220),]
fits[,4]<-NA;

data1A<-rbind(data1,fits);

#########
data1<-split(mydat.df,list(mydat.df$set))[[2]]
names(data1)[2:3]<-c("Time","Observation");
data1<-data1[,-1];#drop set
