// to run to sourceCpp("finitediff_stoch_logistic.cpp")
// then call m<-ode(inits,0.1,100.0,100.0,200,(130- -0)/200)
...
#ifdef IGNORE
inits<-rep(0,201);
inits[14:19]<-c(0.0000756633, 0.0441917, 0.878783, 0.594986, 0.0137156, 0.0000107649)

f1<-function(a){return(ifelse(a<0,0,a));}
m<-apply(m,2,f1)
...
fig <- plot_ly(
  ...
  z = ~m2[-c(1:100),-1],showscale = FALSE)
fig <- fig %>% layout(
    scene = list(
      yaxis = list(showticklabels=FALSE, title='Time'),
      xaxis = list(showticklabels=FALSE, title="x(t)"),
      zaxis = list(showticklabels=FALSE, title="PDF(x(t)")))
...
public:
    ...
    /******************* START OF KEY SECTION = THE MODELLING PART WHICH SHOULD BE CUSTOMIZED **************************/
        ...
        for(int r=1;r<(m_n-1);r++){//e.g. m_n=11 for n=10 steps
          ...
          for(int c=0;c<m_vec2d_xx[r].size();c++){ //add up all the relevant terms using the cofficients stored in m_vec2d[] and m_vec2dh
            ...
            // this is PDE specific - tailor to the equation, this line is for f_xx 
              dxdt[r] += (m_alpha/2.)*(
                m_vec_xgrid[m_vec2dh_xx[r][c]]*m_vec2d_xx[r][c]*x[m_vec2dh_xx[r][c]]/(m_h*m_h)); // f_t = (sigma2/2)*f_xx ...
            }
          ...
          for(int c=0;c<m_vec2d_x[r].size();c++){ //add up all the relevant terms using the cofficients stored in m_vec2d[] and m_vec2dh
           ...
           // this is PDE specific - tailor to the equation, this line is for u_x terms  
              dxdt[r] += -m_alpha*(m_vec_xgrid[m_vec2dh_x[r][c]]*(1.-m_vec_xgrid[m_vec2dh_x[r][c]]/m_beta)*m_vec2d_x[r][c]*x[m_vec2dh_x[r][c]])/m_h; 
                                                            // f_t = ... - theta*f2_x where f2= a x_i (1-x_i/b)f so D[f2,x] etc
            }
      ...
      dxdt[m_n-1] = -1.*(3.*m_beta*m_vec_xgrid[m_n-5]*dxdt[m_n-5]-4.*m_beta*4.*m_vec_xgrid[m_n-4]*dxdt[m_n-4]-9.*m_vec_xgrid[m_n-3]*dxdt[m_n-3]+
        12.*m_vec_xgrid[m_n-2]*dxdt[m_n-2])/(m_vec_xgrid[m_n-1]*((25.-24.*m_h)*m_beta+24.*m_h*m_vec_xgrid[m_n-1]));
